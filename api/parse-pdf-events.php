...
$PYTHON      = trim((string) shell_exec('which python3 2>/dev/null')) ?: 'python3';
...
// Resolve PDF path — must stay inside uploads/
...
if (empty($parsed)) {
    jsonOut(true, 'Keine Events im PDF gefunden', ['imported' => 0, 'skipped' => 0, 'events' => []]);
}
...
foreach ($parsed as $ev) {
    if (!isset($ev['id'], $ev['title'], $ev['start'], $ev['end'])) continue;
    $key = dedupKey($ev);
...
jsonOut(true, $msg, ['imported' => $imported, 'skipped' => $skipped, 'events' => $parsed]);
